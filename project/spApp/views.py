...
def index(request):
    ...
    """
    return render(request, 'index.html') # Renders the index.html page

def signup(request):
    """Handles user registration.
    
    This view processes user registration through a custom user creation form.
    It also logs the user in automatically after successful signup.
    """
    if request.method == 'POST': # Check if the form has been submitted as a POST request
        form = CustomUserCreationForm(request.POST) # Create a form with the data from the post request
        if form.is_valid(): # Check if the form is valid, meaning all fields pass validation rules
            user = form.save() # Save the form data, creating a new user
            login(request, user) # Log the user in
            return redirect('index')  # Redirect to the home page after successful signup
    else: # If the request is a GET request (i.e., the user is accessing the form initially)
        form = CustomUserCreationForm() # If not a post request we create a new empty form
...
def signin(request):
    ...
    """
    if request.method == 'POST': # If the request is a post, meaning the user is trying to log in.
        form = AuthenticationForm(request, data=request.POST) # Create the form to handle login, using data from the post request
        if form.is_valid(): # If form is valid, meaning all required fields have been filled out correctly
            ...
            user = authenticate(username=username, password=password) # Authenticate the user against the database
            if user is not None: # If a user is found with those credentials
                login(request, user) # Login the user in
                return redirect('index') # Redirect the user to home page
    else:  # If the request is a GET request (i.e., the user is accessing the login form initially)
        form = AuthenticationForm() # Create a new empty form for login
    return render(request, 'signin.html', {'form': form}) # Renders the signin template with the default form
...
def signout(request):
    ...
    """
    logout(request) # Logout the user by using the method provided from django
...
def course_detail(request, course_id):
    ...
    return render(request, 'course_detail.html', {'course': course, 'lessons': lessons, 'is_enrolled': is_enrolled}) # Renders the course detail page with the course, lessons and if it is enrolled
...
def lesson_detail(request, course_id, lesson_id):
    ...
    """
    course = get_object_or_404(Course, id=course_id)  # Get the course by it's id or return 404 if not found
    lesson = get_object_or_404(Lesson, id=lesson_id, course=course) # Get the lesson by it's id and course or return 404 if not found
    return render(request, 'lesson_detail.html', {'course': course, 'lesson': lesson}) # Renders the lesson detail template with the course and lesson
...
def enroll_course(request, course_id):
    ...
    return redirect('course_detail', course_id=course_id)  # Redirect to the course detail page
...
def my_courses(request):
    ...
    return render(request, 'my_courses.html', {'enrollments': enrollments})  # Renders the my_courses template with the list of enrollments
...
def course_create(request):
    ...
    if request.method == 'POST': # Check if the request is a post request
        ...
        lesson_form = LessonForm(request.POST) # Create the form for the lesson and gets the data from it
        if course_form.is_valid(): # If the course form is valid
            ...
            if lesson_form.is_valid(): # If the lesson form is valid
        ...
        course_form = CourseForm()  # create a new empty course form
        lesson_form = LessonForm()  # create a new empty lesson form
